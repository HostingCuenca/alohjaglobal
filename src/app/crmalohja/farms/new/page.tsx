...
// Coordenadas aproximadas del centro de cada provincia para seguridad
...
export default function NewFarmPage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [farmersResponse, provincesResponse] = await Promise.all([
        fetch('/api/farmers'),
        fetch('/api/provinces')
      ])
  ...
  const updateFormData = (field: keyof FarmFormData, value: any) => {
    ...
    // Auto-set coordinates when farmer changes (based on farmer's province)
    if (field === 'farmer_id' && value) {
      const selectedFarmer = farmers.find(f => f.id === value)
      if (selectedFarmer) {
        // Find province coordinates
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        router.push('/crmalohja/farms')
      } else {
        setErrors(data.errors || { general: 'Error al crear la finca' })
      }
    } catch (error) {
            ...
            <div className="mb-8 flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">Crear una nueva finca</p>
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Las coordenadas se asignarán automáticamente basadas en la provincia del agricultor
                    </p>
                  </div>
